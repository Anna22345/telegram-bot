...
PHONE_REGEX = r'^\+?\d{10,15}$'  # Допустим, номер телефона содержит от 10 до 15 цифр
...
def plan_selected(call):
    ...
    bot.send_message(call.message.chat.id, "🏢 Enter your organization name:")
...
def validate_email(message):
    ...
    if re.match(EMAIL_REGEX, email):
        user_orders[message.chat.id]["email"] = email
        bot.send_message(message.chat.id, "📞 Enter your phone number:")
        bot.register_next_step_handler(message, validate_phone)
    else:
        bot.send_message(message.chat.id, "⚠️ Invalid email format. Please enter a valid email address:")
        bot.register_next_step_handler(message, validate_email)
...
def validate_phone(message):
    ...
    if re.match(PHONE_REGEX, phone):
        user_orders[message.chat.id]["phone"] = phone
        confirm_order(message)
    else:
        bot.send_message(message.chat.id, "⚠️ Invalid phone number. Please enter a valid phone number (e.g. +1234567890):")
        bot.register_next_step_handler(message, validate_phone)

# Подтверждение заказа с Order ID
...
def final_confirmation(call):
    ...
    del user_orders[call.message.chat.id]  # Удаляем временные данные
...
def pest_catalog_buttons():
    ...
    keyboard.add(InlineKeyboardButton(text="🌍 More Info on Website", url="https://bxb.wus.mybluehost.me/website_01066f10/blog/"))
    return keyboard
